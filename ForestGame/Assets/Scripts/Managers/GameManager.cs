...
public class GameManager : MonoBehaviour
{
    ...
    public void LoadLevel(LevelData level)
    {
        ...
        RoundManager.Instance.SeedInitialWeeds();
        ...
        InteractionManager.Instance.InitializeBudgets();
        ...
        for (int x = 0; x < level.width; x++)
        {
            for (int y = 0; y < level.height; y++)
            {
                ...
                else
                {
                    ...
                    float darkness = Mathf.Clamp01(dist / (maxDist * 0.15f));
                    darkness = Mathf.Pow(darkness, 0.4f);
                    darkness = Mathf.Max(darkness, 0.5f);
